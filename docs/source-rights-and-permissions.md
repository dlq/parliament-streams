...
| Canadian sub-national expansion | `british-columbia-legislature-webcasts`, `saskatchewan-legislative-proceedings`, `manitoba-house-broadcasts`, `northwest-territories-watch-session` | `personal_use_pending_review` | Official legislature webcast and calendar pages recorded in each entry | Link out while explicit third-party playback or reuse terms remain undocumented. Alberta, Prince Edward Island, and Newfoundland and Labrador have separate conditional-reuse records in the catalogue. |
...
| German Landtage | `north-rhine-westphalia-landtag-live`, `baden-wurttemberg-landtag-live`, `bavaria-landtag-plenum-online` | Mixed | Official mediathek/live pages and source terms recorded in each entry | Link out. NRW is `no_third_party_reuse`; Baden-Wurttemberg has conditional mediathek terms; Bavaria remains permission-pending. |
| Spanish autonomous parliaments | `catalonia-canal-parlament`, `valencia-canal-corts`, `andalusia-parliament-tv-live`, `navarre-parliament-live` | Mixed | Official live, agenda, and source-condition pages recorded in each entry | Link out. Catalonia documents conditional reuse; the other three remain permission-pending. |
| Jalisco Canal Parlamento | `jalisco-canal-parlamento` | `personal_use_pending_review` | [Official live page](https://www.congresojal.gob.mx/trabajo/transmisiones_en_vivo) | The direct HLS manifest is technically validated and attributable to the official Congreso de Jalisco player. Native playback follows the catalogue's opt-out posture while explicit reuse terms remain undocumented. |
